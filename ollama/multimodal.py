...
def create_sample_images(image_dir="./sample_images"):
    ...
    draw.text((100, 200), "2024 분기별 매출 현황(억원)", fill=(30, 30, 30))
    ...
    draw.text((10, 15), "상승 추세: Q4 최고치 달성", fill=(100, 0, 0))
...
def create_sample_text_docs():
    ...
    docs = [
        Document(
            ...
            1분기(Q1) 120억원, 2분기(Q2) 185억원, 3분기(Q3) 160억원, 4분기(Q4) 230억원을 기록했으며,
            Q4에 최고치를 달성했습니다.
...
def build_multimodal_index(image_dir: str, text_docs: list[Document]):
    """
    이미지 폴더의 모든 이미지들을 캡셔닝하여 텍스트 문서와 함께 백터 인덱스 구축
    """
    all_docs = list(text_docs)

    # image_dir 안 파일 가져오기
    # image_files모델마다 달라서 안정하게 수정
    ...
    if not image_files:
        ...
        print(f"이미지 캡셔닝 {len(image_dir)}개")
...
def multimodal_answer(vectorstore, question):
    """
    텍스트 + 이미지 검색 결과를 통합하여 멀티모달 최종 답변 생성

    args:
        vectorstore: Chroma 백터 스토어
        question: 사용자 질문
...
def load_or_build_vectorstore(
    image_dir, text_docs, db_path="./db/multimodal.db", force_rebuild=False
):
    """
    벡터스토어가 이미 존재하면 로드하고, 없으면 새로 구축
    """
...
if __name__ == "__main__":
    ...
    vectorstore = load_or_build_vectorstore(
        image_dir=IMAGE_DIR, text_docs=text_docs, force_rebuild=False
    )
